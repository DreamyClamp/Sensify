...
class SensorViewModel(var mSensorType: Int) : ViewModel() {
    ...
    private val _liveValues = MutableStateFlow<FloatArray?>(null)
    val liveValues: StateFlow<FloatArray?> = _liveValues.asStateFlow()

    private val _samplingRateHz = MutableStateFlow(0f)
    val samplingRateHz: StateFlow<Float> = _samplingRateHz.asStateFlow()
    private var mLastPacketTime: Long = 0
...
//            Log.d("rememberChartUiUpdateEvent", "filtered: $filtered, ${mpChartViewManager.sensorType}")
        ...
        viewModelScope.launch {
            sensorPacketFilteredFlow.collect {
                it.values?.let { vals ->
                    _liveValues.value = vals
                }
                val now = System.currentTimeMillis()
                if (mLastPacketTime > 0) {
                    val dt = now - mLastPacketTime
                    if (dt > 0) {
                        val hz = 1000f / dt.toFloat()
                        val currentRate = _samplingRateHz.value
                        _samplingRateHz.value = (currentRate * 0.8f) + (hz * 0.2f)
                    }
                }
                mLastPacketTime = now
...
//                    mModelLineChart
    ...
    override fun onCleared() {
        super.onCleared()
        mChartDataManager?.destroy()
        SensorPacketsProvider.getInstance().detachSensor(mSensorType)
    }
}
